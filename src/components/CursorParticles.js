...
const CursorParticles = ({ screenWidth, hasError }) => {
    ...
    if (hasError)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx("div", { ref: containerRef, className: "fixed inset-0 w-full h-full bg-transparent z-50", style: { pointerEvents: "none" } }), sparkles.length > 0 && (_jsx("div", { className: "fixed z-50 pointer-events-none", style: {
                    left: `${clickPosition.x}px`,
                    top: `${clickPosition.y}px`,
                    transform: "translate(-50%, -50%)",
                }, children: sparkles.map((sparkle) => (_jsx(motion.span, { initial: {
                        x: sparkle.startX,
                        y: sparkle.startY,
                        opacity: 0,
                        scale: 0.5,
