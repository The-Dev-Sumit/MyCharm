...
const Background = ({ hue = 329, xOffset = 0, speed = 1.5, intensity = 1, size = 2, }) => {
    ...
    useEffect(() => {
        ...
        // Cleaned Vertex Shader (no leading newline or extra spaces)
        const vertexShaderSource = "attribute vec2 aPosition;void main() {gl_Position = vec4(aPosition, 0.0, 1.0);}";
        // Cleaned Fragment Shader (single-line for simplicity, full code below)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {vec2 uv = fragCoord / iResolution.xy;uv = 2.0 * uv - 1.0;uv.x *= iResolution.x / iResolution.y;float aspect = iResolution.x / iResolution.y;uv.x = uv.x - uXOffset;float isMobile = step(1.0, aspect);uv.x *= mix(1.5, 1.0, isMobile);uv += 2.0 * fbm(uv * uSize + 0.8 * iTime * uSpeed) - 1.0;float dist = abs(uv.x);vec3 baseColor = hsv2rgb(vec3(uHue / 360.0, 0.7, 0.8));vec3 col = baseColor * pow(mix(0.0, 0.07, hash11(iTime * uSpeed)) / dist, 1.0) * uIntensity;col = pow(col, vec3(1.0));fragColor = vec4(col, 1.0);}
...
`;
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "fixed top-0 left-0 w-full h-screen z-[-1]" }));
};
